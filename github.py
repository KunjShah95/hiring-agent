...
def _create_cache_filename(api_url: str, params: dict = None) -> str:
    ...
    # Replace chars invalid on Windows
    invalid_chars = '<>:"|?*'
...
def fetch_user_contributions(github_url: str) -> Dict:
    """Fetch real PR/issue contributions across ALL repos using GitHub Search API."""
    username = extract_github_username(github_url)
    if not username:
        return {}

    print(f"🔍 Searching for PR contributions by {username} across all repos...")

    prs = []
    issues = []
    repos_contributed = set()
    orgs_contributed = set()
    own_repo_prefix = f"{username}/"

    for pr_type, search_type in [("pr", "pr"), ("issue", "issue")]:
        api_url = "https://api.github.com/search/issues"
        params = {
            "q": f"author:{username} type:{search_type}",
            "per_page": 100,
            "sort": "created",
            "order": "desc",
        }
        ...
        if status_code == 200:
            ...
            for item in items:
                ...
                repo_name = repo_full.replace("https://api.github.com/repos/", "")
                org_name = repo_name.split("/")[0] if "/" in repo_name else ""
                state = item.get("state", "")
                created = item.get("created_at", "")
                title = item.get("title", "")
                html_url = item.get("html_url", "")
                is_own_repo = repo_name.lower().startswith(own_repo_prefix.lower())
                ...
                if pr_type == "pr":
                    ...
                    if pull_request:
                        ...
                        entry["merged"] = merged
                        prs.append(entry)
                        if merged and not is_own_repo:
                            repos_contributed.add(repo_name)
                            if org_name:
                                orgs_contributed.add(org_name)
                else:
                    pull_request = item.get("pull_request")
                    if not pull_request:
                        issues.append(entry)

    external_prs = [p for p in prs if not p.get("is_own_repo")]
    own_prs = [p for p in prs if p.get("is_own_repo")]
    external_merged = [p for p in external_prs if p.get("merged")]

    print(f"  Total PRs: {len(prs)} ({len(own_prs)} own repos, {len(external_prs)} external repos)")
    print(f"  Merged PRs to external projects: {len(external_merged)}")
    print(f"  Issues: {len(issues)}")
    print(f"  External repos contributed to: {len(repos_contributed)}")
    if orgs_contributed:
        print(f"  Orgs: {', '.join(sorted(orgs_contributed)[:10])}")

    return {
        "total_prs": len(prs),
        "own_repo_prs": len(own_prs),
        "external_prs": len(external_prs),
        "merged_prs": len([p for p in prs if p.get("merged")]),
        "external_merged_prs": len(external_merged),
        "open_prs": len([p for p in prs if p.get("state") == "open"]),
        "total_issues": len(issues),
        "repos_contributed": sorted(repos_contributed),
        "orgs_contributed": sorted(orgs_contributed),
        "recent_prs": sorted(prs, key=lambda x: x.get("created_at", ""), reverse=True)[:10],
    }
...
def fetch_and_display_github_info(github_url: str) -> Dict:
    ...
    contributions = fetch_user_contributions(github_url)
