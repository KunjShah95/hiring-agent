...
def convert_github_data_to_text(github_data: dict) -> str:
    ...
    if "contributions" in github_data:
        contribs = github_data["contributions"]
        github_text += f"\n=== EXTERNAL CONTRIBUTIONS (PRs & Issues to OTHER projects) ===\n"
        github_text += f"Total PRs authored: {contribs.get('total_prs', 0)} "
        github_text += f"({contribs.get('own_repo_prs', 0)} to own repos, {contribs.get('external_prs', 0)} to external repos)\n"
        github_text += f"Merged PRs to external projects: {contribs.get('external_merged_prs', 0)}\n"
        github_text += f"Open PRs: {contribs.get('open_prs', 0)}\n"
        github_text += f"Total Issues authored: {contribs.get('total_issues', 0)}\n"
        github_text += f"External repos contributed to: {len(contribs.get('repos_contributed', []))}\n"

        orgs = contribs.get("orgs_contributed", [])
        if orgs:
            github_text += f"Organizations contributed to: {', '.join(orgs)}\n"

        recent_prs = contribs.get("recent_prs", [])
        if recent_prs:
            github_text += f"\nRecent PRs (up to 10):\n"
            for pr in recent_prs[:10]:
                merged = "✅ Merged" if pr.get("merged") else "📋 Open"
                where = "own repo" if pr.get("is_own_repo") else "external"
                github_text += f"  - [{pr['repo']}] {pr['title']} ({merged}, {where}, {pr.get('created_at', '')[:10]})\n"
        github_text += "\n"
